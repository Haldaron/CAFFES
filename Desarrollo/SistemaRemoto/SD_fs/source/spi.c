 ...
 */
uint8_t spi_initialize(spi_dev_t *dev, GPIO_Type *base,uint32_t pin){
...
uint8_t spi_transfer(spi_dev_t *dev, uint8_t data_out[], uint8_t data_in[], uint8_t transfer_size){
	...
	SPI_MasterTransferCreateHandle(SPI_MASTER_BASE, &spi_handle, masterCallback, dev);
    ...
    status = SPI_MasterTransferNonBlocking(SPI_MASTER_BASE, &spi_handle, &masterXfer);
