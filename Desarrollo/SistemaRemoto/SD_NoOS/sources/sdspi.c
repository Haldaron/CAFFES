...
void sdspi_reset(sdspi_dev_t *dev){
	...
	SPI_MasterTransferCreateHandle(SDSPI_MASTER_BASE, &spi_handle, masterCallback, dev);
    ...
    SPI_MasterTransferNonBlocking(SDSPI_MASTER_BASE, &spi_handle, &masterXfer);
...
uint8_t sdspi_transfer(sdspi_dev_t *dev, uint8_t data_out[], uint8_t data_in[], uint8_t transfer_size){
    ...
    status = SPI_MasterTransferNonBlocking(SDSPI_MASTER_BASE, &spi_handle, &masterXfer);
    while(dev->masterFinished!=true && i<=MAX_SPI_TRANS_ITER){
    	i++;
    }
    GPIO_SetPinsOutput(SD_BASE, mascara );		//pinCS=1
